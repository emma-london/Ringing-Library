...
- **[ADR-0022](docs/adr/ADR-0022-dynamic-method-library-loader.md)** *(Accepted; partially amends ADR-0015)* — **dynamic method-library loader for power users.** An **opt-in loader on its own subpath** (**`ringing-lib-ts/cccbr-methods`**), *outside* the zero-I/O core, that **fetches direct from CCCBR over HTTPS** (`methods.cccbr.org.uk/text`, **no project-hosted mirror**), **parses the raw Text client-side** (promoting the existing pure `parse-cccbr-text` parser out of `scripts/` into shipped `src/`, shared with the refresh script), and returns a `MethodLibrary` **tagged with provenance** (upstream "Generated on" marker + fetch date) so the app owns the refresh-or-not decision (one-and-done for most). **Amends, not supersedes, ADR-0015:** the bundled snapshot / `STANDARD_SET` stays the default and the offline guarantee; a failed fetch degrades to it (ADR-0014 spirit — loader provides the mechanism, app owns the UX). First shipped network I/O in the package, quarantined to the subpath so the **core's zero-I/O guarantee (ADR-0001) is unchanged**. **Resolved 2026-07-11:** **CORS verified green** (cross-origin `fetch` from the deployed test bench origin `emma-london.github.io` to the CCCBR Text endpoints returns readable `type:"cors"` 200s — fallback still load-bearing since CORS is CCCBR's to change); **granularity** = per (file-class, stage) primitive — one CCCBR file = one fetch (`CCCBR_Surprise6.html` = Surprise Minor; the motivating case is a ringer prepping a peal of one new method) — plus an opt-in whole-stage merge helper, with the **class×stage availability map** shipped for pickers (the 8 fetchable file-classes, *not* the 10-value `MethodClassification`); **caching** = pure-fetch, app owns the cache (keyed on `provenance.upstreamGenerated`); **merge** = app-side spread over the iterable `MethodLibrary`, de-dupe by `id`. Jump/Dynamic excluded (extended notation). **Built 2026-07-11** — `src/cccbr/` (`parse-text.ts` shared parser + `index.ts` loader), subpath `ringing-lib-ts/cccbr-methods`, v1.3.0, 12 loader tests; see Phase 4aa in the roadmap.
...
- **[ADR-0023](docs/adr/ADR-0023-standard-set-seed-hash-guard.md)** *(Accepted)* — **seed-hash guard against a stale bundled standard set.** `src/data/method-library/standard-set.json` is *generated* from `standard-set-seed.json` by `data:refresh`, but `npm run build` (tsc) only **copies** it — it never re-resolves the seed — so a seed edited without a re-refresh silently ships stale (which it did once: seed updated, built in CI, published to npm, but the generation step never re-ran; `data:refresh` fixed it after the fact). A full regenerate-and-diff in CI is awkward — `data:refresh` needs the network (CCCBR) and folds in unrelated upstream drift. Instead: **record a canonical, network-free hash of the seed's method membership** (`scripts/seed-hash.mjs`, id + normalised title + stage; order/format-independent, `normTitle` shared with the refresh script) in **`src/data/method-library/standard-set.seedhash`** — written by `refresh-method-library.mjs`, verified by **`npm run data:check`** (`scripts/check-standard-set.mjs`) on a new **`.github/workflows/ci.yml`** (push/PR to `main`, alongside `npm test` + build) and as **`prepublishOnly`** (so the manual `npm publish` path — the one that failed — is guarded too). Rejected: regenerate-and-diff (network + false positives), raw-byte hash (cosmetic churn), hashing the output (misses the actual failure mode), CI-only. Upstream-freshness (a scheduled full refresh) is separate, out of scope. Verified: passes in sync, fails on an unrefreshed seed edit, passes after revert. **Amended 2026-07-12:** `prepublishOnly` is now `data:check && build`, so a manual `npm publish` rebuilds `dist` from source (guard first — cheap, network-free — then `tsc`); deliberately still **not** `data:refresh` (no network / no upstream drift at publish). Verified the chain emits `dist/index.js`, the `./data/standard-set` subpath, and the bundled `standard-set.json`.
...
| `src/cccbr/parse-text.ts` | **CCCBR Text-format parser** (ADR-0015; promoted from `scripts/lib/parse-cccbr-text.mjs` into shipped `src/` by ADR-0022 so the runtime loader and the refresh script share one copy) — pure, I/O-free: raw file text + (class, stage) → Lean `MethodLibraryEntry` objects. Lead-head code/row discrimination, full-title reconstruction, Plain→Bob/Place split, canonical notation from tab-separated tokens; also `extractUpstreamDate` (the "Generated on" marker) and the `CccbrFileClass` type |
| `src/cccbr/index.ts` | **Dynamic method-library loader** (Phase 4aa / ADR-0022) — the `ringing-lib-ts/cccbr-methods` subpath export, the package's one opt-in network path (off the zero-I/O root). `loadMethods(fileClass, stage)` (single-file primitive), `loadStage(stage)` (whole-stage merge), the class×stage availability map (`CCCBR_AVAILABILITY`, `classesForStage`, `stagesForClass`, `isAvailable`), `LibraryProvenance`/`LoadedLibrary`, `CccbrLoadError`, injectable `FetchLike`. Fetches CCCBR Text over HTTPS, parses via `parse-text.ts`, tags provenance; throws on failure so the app falls back to `STANDARD_SET` |
| `scripts/seed-hash.mjs` | **Canonical seed hash** (ADR-0023) — shared by the refresh script (writes) and the guard (verifies), so they can't drift. SHA-256 of the seed's *membership* (id + `normTitle`-normalised name + stage), order/format-independent; changes exactly when the seed→snapshot resolution would |
| `scripts/check-standard-set.mjs` | **Seed-hash guard** (`npm run data:check`, ADR-0023) — network-free: recomputes the seed hash and fails if it differs from the recorded `standard-set.seedhash`, pointing at `npm run data:refresh`. Runs on CI and as `prepublishOnly` |
| `src/data/method-library/standard-set.seedhash` | **Recorded seed hash** (ADR-0023) — written by `data:refresh` next to `standard-set.json`; the committed baseline `data:check` compares against |
...
| `.github/workflows/ci.yml` | GitHub Actions CI (ADR-0023): on push/PR to `main`, runs `npm run data:check` (the seed-hash guard), `npm test`, `npm run build`. The project's first CI beyond Pages deployment |
...
| `src/tests/parse-cccbr-text.test.ts` | 8 tests — the CCCBR Text parser (ADR-0015; now imported from `src/cccbr/parse-text.ts`) against **real CCCBR rows**: title reconstruction (Plain/Surprise/Principle/Little), Plain→Bob/Place, token→notation, coded vs row lead heads, symmetry/little; cross-checks parsed notation rebuilds the hand-verified lead heads (Grandsire `1253746`, Plain Bob `1352746`) |
| `src/tests/cccbr-methods.test.ts` | 12 tests — the dynamic loader (ADR-0022) with an **injected fake fetch** (no network): availability map (8 file classes, `classesForStage`/`stagesForClass`/`isAvailable`), `loadMethods` fetches exactly one file + parses + tags provenance + builds a real `MethodLibrary`, URL stem (`Treble Bob`→`TrebleBob`), `RangeError` on an unpublished (class,stage) with no dead fetch, `CccbrLoadError` on non-2xx and on a rejected fetch (with `cause`/`url`); `loadStage` fetches every class of a stage and merges, and propagates a `CccbrLoadError` if any class file fails |
...
**294 tests, all passing.** Clean `tsc` build.
...
**Phase 4aa — dynamic method-library loader ([ADR-0022](docs/adr/ADR-0022-dynamic-method-library-loader.md), Accepted 2026-07-11).** A method-data track, named "4aa" because it extends Phase 4a's method-database work and sits outside the engine sequence (4b/5). A power-user capability sitting on the method-data side, not the engine side, so it does **not** block or depend on Phase 4b/5. Ship the **`ringing-lib-ts/cccbr-methods`** subpath: promote the pure `parse-cccbr-text` parser into `src/` (shared with the refresh script), a per-(file-class, stage) loader over `https://methods.cccbr.org.uk/text` returning `{ library, provenance }`, an opt-in whole-stage merge helper, the class×stage availability map, and tests against the live endpoints. Pure-fetch (a built-in offline cache is deferred to a Phase 5 spike — IndexedDB is Node-incompatible, so it needs a pluggable storage-adapter seam; see Phase 5); a failed fetch degrades to the bundled `STANDARD_SET`. CORS already verified green. **✅ BUILT (2026-07-11).** Shipped as the `ringing-lib-ts/cccbr-methods` subpath (`src/cccbr/index.ts`): `loadMethods(fileClass, stage)` (single-file primitive), `loadStage(stage)` (whole-stage merge), `CCCBR_AVAILABILITY` + `classesForStage`/`stagesForClass`/`isAvailable` (picker map), `LibraryProvenance`/`LoadedLibrary`, `CccbrLoadError`. The pure parser moved to `src/cccbr/parse-text.ts` (shared with the refresh script, which now imports the built copy and `data:refresh` builds first). Off the root entry; version bumped to **1.3.0** (additive MINOR). 12 loader tests with an injected fake fetch (URLs, provenance, whole-stage merge, `RangeError` on unpublished (class,stage), `CccbrLoadError` on network/HTTP failure); the ported parser's 8 tests still pass. **294 tests, clean `tsc`.** Live end-to-end unverified in-sandbox (no outbound network to CCCBR, per ADR-0015) — the error path is confirmed (network failure → `CccbrLoadError`); real fetch to run in a networked env / the app.

**Phase 5 — Orchestration and scale.** Admission controller, cost model, server handoff UX, cached composition corpus (ADR-0001 "later"). Also carries the **Stedman Doubles call-model placeholder** left by ADR-0009: `stedmanCalls(5)` currently throws rather than implementing the genuinely different (stage-5) call structure — picked up here, not before. **Also carries a spike on a built-in offline method-library cache** for the `cccbr-methods` loader (ADR-0022, deferred 2026-07-11): revisit the deferred IndexedDB cache via a **pluggable storage-adapter seam** that keeps the loader Node-compatible (IndexedDB is browser-only) — motivated by mobile/no-coverage field use, where the app should keep working with whatever was last loaded. Spike measures impact (stored sizes, cold/warm loads, on-phone offline behaviour) and sets the invalidation policy; outcome is its own ADR or an ADR-0022 amendment.
