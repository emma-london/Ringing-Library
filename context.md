...
**240 tests, all passing.** Clean `tsc` build.
...
**Phase 4a — WASM engine core ([ADR-0013](docs/adr/ADR-0013-phase-4-prework-and-split.md)).** Language spike (Rust/wasm-pack or AssemblyScript) on the ranked-lead-head DFS + bitset truth kernel; port the remaining validated prototype techniques (reachability DP, meet-in-the-middle, Q-set generative search, snap-finish handling); validate by **live diff against the C++ prototypes**. Deliverable: a working, verified WASM module, callable directly with no `Executor`/chunking yet — an independent checkpoint. **Also scoped in (2026-07-03):** vendor a **bundled CCCBR method-library snapshot** — not a live fetch — so `MethodLibrary` can offer the full method set without an app needing connectivity just to look up a method ([ADR-0015](docs/adr/ADR-0015-cccbr-method-library-data-source.md), Draft; mechanics — refresh script, format, lazy-loading, staleness marker — to resolve when 4a starts).
...
**Pre-4 is complete** (2026-07-02) — ADR-0008, ADR-0009, and the `BELL_NAMES` extension are all done (see Roadmap above). **Phase 4a** (WASM engine core) is next.
