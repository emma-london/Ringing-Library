...
**210 tests, all passing.** Clean `tsc` build.
...
**Phase 4 — Search engine + the execution plumbing it serves.** WASM engine (Rust/wasm-pack or AssemblyScript spike first), plus everything that only exists to serve a searcher: `Executor` (`Local`/`Worker`/`Remote`), per-call budget, deterministic search mode, resumable chunk protocol, streaming/progress/cancel. Validated by **live diff against the C++ prototypes** (not frozen vectors). Deferred deliberately — search is rarely used and cheap to test side-by-side; space is left for it, it is not pre-built. No throwaway TS searcher. **Note (ADR-0011 / ADR-0012):** a *bounded* app-only searcher (`src/search.ts`, capped/shortest-first, naive DFS) now exists in Phase 3 to serve the test bench's Search tab — `searchTouches` for lead-end methods and `searchStedmanTouches` for Stedman's six-based calling. Neither is the scalable engine this phase deferred — no reachability DP / rank-bitset / MITM — and both are expected to be **superseded** here: replace their bodies with the WASM engine behind the same `SearchReport` interface. If Phase 4 finds itself extending those DFS bodies rather than replacing them (or adds a third principle family and wants to share a core), revisit ADR-0011 / ADR-0012.
