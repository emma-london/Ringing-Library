...
- **[ADR-0015](docs/adr/ADR-0015-cccbr-method-library-data-source.md)** *(Draft)* — CCCBR method library data source. The full CCCBR library ships as a **bundled static snapshot, not a live fetch** — Emma's direction, since the upstream data changes only ~monthly, and fetching it live would reintroduce a network dependency into what's otherwise a zero-I/O core. `MethodLibrary` needs no change; this is only about where the array's data comes from. Open questions for Phase 4a: refresh-script mechanics, storage format, lazy-loading by stage/class (phone-performance relevant), a staleness marker, and keeping `STANDARD_METHODS` distinct from the full library (the former stays small and hand-verified for the truth corpus; the latter is bulk, unverified-per-entry application data).
...
**Phase 4a in progress — generic engine core built (2026-07-03).** The language
...
Royal (10). Principles (Stedman) stay out of scope. **Still open in 4a:** the CCCBR
method-library snapshot (ADR-0015), deferred to a follow-up session. Rust/wasm-pack
remains the recorded future drop-in behind the same seam. See Roadmap below.
...
**267 tests, all passing.** Clean `tsc` build.
...
- Truth verified against the ADR-0008 corpus (`src/tests/fixtures/known-touches.json`, generating `docs/example-touches.md`): 11 fixtures across Grandsire/Plain Bob/Stedman — verdicts and lengths (70 / 84 / 97 snap / 224 / 84 / 168 / 252) all match; false cases report their repeated rows with line numbers
